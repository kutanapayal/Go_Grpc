...
.DEFAULT_GOAL := help
.PHONY: greet blog a help
project := greet a blog

all: $(project) ## Generate Pbs and build

greet: $@ ## Generate Pbs and build for greet
a: $@ ## Generate Pbs and build for a
...
clean: clean_greet clean_a clean_blog ## Clean generated files
	${RM_F_CMD} ssl/*.crt
	${RM_F_CMD} ssl/*.csr
	${RM_F_CMD} ssl/*.key
	${RM_F_CMD} ssl/*.pem
...
clean_a: ## Clean generated files for a
	${RM_F_CMD} a/${PROTO_DIR}/*.pb.go
